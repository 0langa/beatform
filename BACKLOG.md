...
Last reconciled: **2026-08-14** (post v2.95.0 shipment and headless installed-update verification)
...
| Branch                  | `main`; release tag `v2.95.0` points at `09d24ec`; nothing follows the tag but this reconciliation                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Source version          | `2.95.0` in all five version-bearing files; `node scripts/bump-version.mjs --verify` passed inside the release run                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Release commit / tag    | `v2.95.0` points at release commit `09d24ec` ("Routes you can mute, meters that tell the truth")                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Latest public release   | `v2.95.0`, published 2026-08-14; downloaded installer and `SHA256SUMS.txt` agree on SHA-256 `b8065edf16fc179ff7186e29ebe5d4acf1c09986a7307e89c888fadd5f5a3d20` (59,906,985 bytes); live `latest.json` serves `2.95.0`, names the v2.95.0 setup asset, and carries a 420-character signature                                                                                                                                                                                                                                                    |
...
| CI / release            | `CI` green on every commit through `09d24ec` (run `31797405899`); `Release installers` run `31797408687` completed `success` and published v2.95.0; `npm audit` reports 0 vulnerabilities                                                                                                                                                                                                                                                                                                                                                      |
| Installed desktop app   | `2.95.0` (`Beatform.exe` `ProductVersion` and `FileVersion`). Updated 2026-08-14 headlessly with the SHA-verified release installer and the updater's own arguments (`/P /UPDATE`), the flow proven for 2.93.0; the isolated-profile runtime smoke passed on its first run against the updated bundle: `http://tauri.localhost/`, title `Beatform`, one canvas, WebGPU available                                                                                                                                                               |
| Uninstall registry      | `2.95.0` exactly (ALIGN-002 check, straight from the installer — no heal needed). The self-heal mechanism was proven LIVE on the 2.93.0 binary (sentinel write, one boot, healed readback); that probe also exposed and corrected a false "verified 2.92.1" claim in the 2026-08-12 row — the pre-update hive actually read `2.82.0`. No code defect; a truth-audit miss, corrected 2026-08-13                                                                                                                                                 |
