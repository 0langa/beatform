...
Last reconciled: **2026-08-13** (post v2.94.0 shipment and headless installed-update verification)
...
| Branch                  | `main`; release tag `v2.94.0` points at `02f602c`; nothing follows the tag but this reconciliation                                                                                                                                                                                                                                                                                                                                                                                            |
| Source version          | `2.94.0` in all five version-bearing files; `node scripts/bump-version.mjs --verify` passed inside the release run                                                                                                                                                                                                                                                                                                                                                                            |
| Release commit / tag    | `v2.94.0` points at release commit `02f602c` ("One guide, and exports that live to the end")                                                                                                                                                                                                                                                                                                                                                                                                  |
| Latest public release   | `v2.94.0`, published 2026-08-13; downloaded installer and `SHA256SUMS.txt` agree on SHA-256 `11493dc440c405a3a29b793dcd36fb263fdb3b77091dd9be5bdf40a32f052a67` (59,891,579 bytes); live `latest.json` serves `2.94.0`, names the v2.94.0 setup asset, and carries a 420-character signature                                                                                                                                                                                                   |
...
| CI / release            | `CI` green on every commit through `02f602c` (run `31749110307`); `Release installers` run `31749112055` completed `success` and published v2.94.0; `npm audit` reports 0 vulnerabilities                                                                                                                                                                                                                                                                                                     |
| Installed desktop app   | `2.94.0` (`Beatform.exe` `ProductVersion` and `FileVersion`). Updated 2026-08-13 headlessly with the SHA-verified release installer and the updater's own arguments (`/P /UPDATE`), the flow proven for 2.93.0; the isolated-profile runtime smoke passed on its first run against the updated bundle: `http://tauri.localhost/`, title `Beatform`, one canvas, WebGPU available                                                                                                              |
| Uninstall registry      | `2.94.0` exactly (ALIGN-002 check, straight from the installer — no heal needed). The self-heal mechanism was proven LIVE on the 2.93.0 binary (sentinel write, one boot, healed readback); that probe also exposed and corrected a false "verified 2.92.1" claim in the 2026-08-12 row — the pre-update hive actually read `2.82.0`. No code defect; a truth-audit miss, corrected 2026-08-13                                                                                                |
