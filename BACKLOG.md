...
Last reconciled: **2026-08-15** (post v2.98.0 shipment and headless installed-update verification)
...
Time-sensitive values below were checked on 2026-08-13:

| Fact                    | Verified state                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Repository              | `0langa/beatform`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Branch                  | `main`; release tag `v2.98.0` points at `c6ad866`; nothing follows the tag but this reconciliation                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Source version          | `2.98.0` in all five version-bearing files; `node scripts/bump-version.mjs --verify` passed inside the release run                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Release commit / tag    | `v2.98.0` points at release commit `c6ad866` ("All themes, one door"); v2.97.1 ("Nothing is ever silently discarded") shipped between them                                                                                                                                                                                                                                                                                                                                                                                                     |
| Latest public release   | `v2.98.0`, published 2026-08-15; installer verified against `SHA256SUMS.txt` inside the release run; live `latest.json` serves `2.98.0`, names the v2.98.0 setup asset, and carries a 420-character signature                                                                                                                                                                                                                                                                                                                                  |
| Open GitHub issues      | 0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Open pull requests      | 0. Dependabot #13 windows-core 0.62 and #15 webview2-com 0.39 CLOSED 2026-08-14: the upstream block was re-verified live (wry through 0.56.1, released 2026-08-13, still pins `webview2-com ^0.38` / `windows-core ^0.61`), and `dependabot.yml` now carries lockstep ignore rules (minor/major only — patch CVE fixes still arrive; commit `8a3f8f2`), so the pair stays closed until the tauri/wry stack moves, at which point BOTH get bumped together by hand. Rationale posted on both PRs. Earlier merges (#19, #20, #21, #14) unchanged |
| CI / release            | `CI` green through `c6ad866`; `Release installers` completed `success` for v2.97.1 and v2.98.0 alike; `npm audit` reports 0 vulnerabilities                                                                                                                                                                                                                                                                                                                                                                                                    |
| Installed desktop app   | `2.98.0` (`Beatform.exe` `ProductVersion` and `FileVersion`). Updated 2026-08-15 headlessly with the SHA-verified release installer and the updater's own arguments (`/P /UPDATE`), the flow proven for 2.93.0; the isolated-profile runtime smoke passed on its first run against the updated bundle: `http://tauri.localhost/`, title `Beatform`, one canvas, WebGPU available                                                                                                                                                               |
| Uninstall registry      | `2.98.0` exactly (ALIGN-002 check, straight from the installer — no heal needed). The self-heal mechanism was proven LIVE on the 2.93.0 binary (sentinel write, one boot, healed readback); that probe also exposed and corrected a false "verified 2.92.1" claim in the 2026-08-12 row — the pre-update hive actually read `2.82.0`. No code defect; a truth-audit miss, corrected 2026-08-13                                                                                                                                                 |
| Running desktop app     | Not running during this reconciliation (smoke boots used isolated profiles and exited)                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- [x] C5 Step-grid follow-through (post-chain gap, found 2026-08-15). **ALL
      THREE remainders now closed — (a) via E5, (b) merged registry-side
      2026-08-16, (c) shipped 2.99.0 — nothing open.** The
      ...
      **Already fixed, uncredited — cross-referenced to E5 in the
      ...
      (b) registry-side rule extension. **DONE 2026-08-16 — gallery PR #2
      MERGED**: `gen-param-specs.mjs` emits post/motion/modAmount/syncTrio
      maps, `validate.mjs` gains `checkSliderGrids`/`checkSyncTrio`,
      `schema/param-specs.json` regenerated from app main (17 presets /
      483 specs). Byte-neutral against all 18 live entries proven, and a
      detune test produced 95 failures proving the checks bite.
...
- [x] P-6 follow-up (found in review): **CLOSED 2026-08-16 (2.100.0,
      dock-gate lane) — `scripts/gallery-builtin-shots.mjs` is exactly the
      "proper CDP-driven capture script" this entry asked for**: bare
      Chrome/Edge CDP against Vite on :1427, applies each `FACTORY_THEMES`
      entry via the new `__factoryThemes` dev hook, blanks `trackMeta`
      (Cover Story leak), shoots the canvas to webp. Regenerated all 13 and
      visually compared: identical looks, not byte-identical (live
      audio-reactive capture, documented in the script) — so the script
      landed alone and the shipped assets stayed untouched. Re-capture
      protocol after a factory-theme edit: run the script, eyeball the
      diff, commit the changed webps with the theme change.
      ORIGINAL: **built-in thumbnail drift is
      unenforced.** The 13 bundled captures
      (`src/assets/gallery-builtins/*.webp`) were taken by hand against a
      running app, not generated by a repeatable script the way
      `gallery-seed-shots.mjs` covers the community registry's seed
      content — nothing re-renders or even flags them if a factory theme's
      params/post/mods/Builder stack change later, so a card could
      silently start showing a stale look. Before the next factory-theme
      edit: either re-capture by hand and eyeball the diff, or build a
      proper CDP-driven capture script first (same pattern as
      `gallery-seed-shots.mjs`, applying each `FACTORY_THEMES` entry via
      `applyTheme` and shooting the canvas).
...
### Track E — Hardening burn-down (from the audit register)

**TRACK CLOSED 2026-08-16 (2.100.0).** E1–E5 all [x]: the last open
thread anywhere in the track was E2 gap (b), closed by the deep-state
fuzz pass — see E2's head note. Nothing in Track E remains scheduled.
...
- [x] E2 Severity-2 waves per domain (state → UI → audio/export →
      platform → render), each wave gated + released. **CLOSED 2026-08-16
      (2.100.0): gap (b) — the deep-state remainder — was the last open
      thread, and its fuzz/mutation pass landed** (`b3b69d3`):
      `modMatrixFuzz.test.ts` (11 properties), `frameResolveFuzz.test.ts`
      (7), `storeActionsFuzz.test.ts` (9) — 27 properties, every one
      mutation-checked red→green, seven real fixes shipped with them
      (proto-key mod source resolving to object garbage, non-finite stem
      values reaching route math, `laneValue` NaN-time, `setParam`
      accepting non-finite, `setPost`/`setMotion` bypassing validators,
      `updateModRoute` unvalidated patch, and — found by review, closed in
      the dock-gate lane — `setModRouteAmountsForParam` missing the same
      finite guard). Behavior preservation traced for each fix.
      ...
      **Gap (b) was the LAST open thread keeping E2's checkbox open**
      ...
      `src/state/store.ts` **landed in 2.100.0** — see the closure note at
      this entry's head for the full record.
...
- [x] E4a **DONE 2026-08-13 — the watchdog can no longer kill a healthy
      ...
      **Shipped in v2.94.0** (2026-08-13); the owner's next installed-app
...
- [x] F2 DONE v2.73.0 — timeouts root-fix landed; rerun-before-believing
      ...
      SWEPT 2026-08-11, LANDED 2026-08-12 — the root-fix had been applied
      suite by suite as each one went red, which is not a fix, it is a queue:
      `featurePipelineFuzz` only got budgets in v2.89.0, and `realtimeSource`
      carried a per-`it` budget that covered four heavy tests and left three
      more in the same class on the 5 s default. Measured every `src/audio`
      test under a 2:1-oversubscribed pool and gave describe-level 30 s
      budgets to everything within 4x of the default: `realtimeSource`
      (4.2 s worst), `syncLatency`, `offlineSource`, `engineGraph`,
      `dsp/truepeak`. GATES.md §1 now states the rule (describe, not `it`)
      instead of naming one file. Provenance: the sweep was authored
      2026-08-11 in a session worktree that was never merged; recovered from
      `claude/sad-meitner-2b368c`'s dirty tree during the 2026-08-12 baseline
      reconciliation and landed with fresh green gates.
...
- [x] H16 **DONE 2026-08-09 — both gaps closed, and the repaired auditor was
      ...
      **Q8 EXECUTED 2026-08-16 (2.100.0, dock-gate lane):** dock overflow
      is now a hard failure inside `assertRuntime`, same `failures[]` path
      as compile/GPU/spectrum errors — nonzero exit, per-finding diagnostic
      lines (dock end, width, page, element, overflow px, text snippet)
      printed before the throw. The feared fallout DID NOT EXIST: full
      matrix, 280/280, **zero** overflow findings at both dock ends
      (352px/692px) across all 8 pages, so the promotion shipped alone
      with nothing to fix. H16 has no remaining open question.
...
- **2.99.0 — SHIPPED 2026-08-16** ("The song draws itself") — Spectro
  Falls (P-19 entry 1) + the ledger truth sweep + the smalls as planned:
  toast-stack cap, sync-trio 0.002 (C5(c)), FEAT-004 follow-up bundle
  (stage-transition progress, measured-RTF estimates, language picker —
  the Vulkan benchmark re-scoped to a toolchain spike, see FEAT-004).
  Installed and verified headless.
- **2.100.0 — SHIPPED 2026-08-16** — Hardening close, delivered in full:
  registry-side step-grid rule extension MERGED (gallery PR #2,
  byte-neutral vs all 18 entries, detune test proves it bites), dock
  overflow now a hard-failing matrix gate (H16 Q8 — fallout: NONE,
  280/280 clean at both dock ends), E2 gap (b) deep-state fuzz pass (27
  mutation-checked properties, 7 real fixes), FEAT-005's ProRes
  deep-color pipe device-proven (12-bit native, straight alpha 1.997,
...
SVT-AV1; 752 distinct decoded levels proven vs the 256 ceiling). Full
record: ARCHIVE at the bottom of this file.

**ProRes deep-color follow-up: DONE 2026-08-16 (2.100.0).** The last
FEAT-005 thread. ProRes 4444 now rides the same rgba64le deep tap as
AV1 instead of the 8-bit PNG pipe — with one lane-specific twist the
AV1 side doesn't have: ffmpeg treats rawvideo rgba64le alpha as
STRAIGHT, and the canvas tap is premultiplied, so
`deepFrameToStraightRgba64` (webgpuRenderer.ts) un-premultiplies in u16
space before the pipe (`deepStraightAlpha` threaded through
exportCore/videoExporter/buildExportOptions; TIMELESS in the segment
key census). Zero WGSL changes. **Device smoke (RESULT.json, run
twice):** 2441 distinct decoded luma levels — real 12-bit, since
prores_ks stores ProRes 4444 at native 12-bit regardless of the
requested `yuva444p10le` (decoder reports `yuva444p12le`; comment truth
recorded in prores.rs); straight-alpha confirmed at ratio 1.997 (a
premul bug reads ~1.0); byte-identical files across runs
(`dea2ef52…`, matching the AV1 lane's determinism precedent). Harness:
`scripts/deepcolor-verify.mjs --prores` (`9bea420`).
...
| CI sidecar-fetch transient retry   | Landed 2026-08-12 (branch commit, unreleased) after CI run 31643439746's `rust` job died on a passing "Download failed: HTTP 503" from the ffmpeg release host — the fetch scripts threw on the first non-OK response. All three (`fetch-ffmpeg`, `fetch-whisper`, `fetch-onnxruntime`) now share `scripts/lib/download.mjs`: four bounded attempts with 2 s/8 s/30 s backoff, retried ONLY for failures that can heal (HTTP 5xx, 429, dropped connections incl. mid-body); a 404 (a vanished pin — see the row above), other 4xx and local fs errors stay immediately fatal, and SHA-256 verification is untouched (callers still hash after download, before install). Evidence, all against the real scripts via an injected fetch shim: 404 fails in 0.15 s with zero retries; sustained 503 makes exactly 4 attempts over 40.2 s then rethrows the original message; ffmpeg recovers after 2×503 into a full sha256-verified install; onnxruntime recovers from an injected ECONNRESET; and the UNPATCHED whisper run hit a genuine mid-download `UND_ERR_SOCKET` that the retry absorbed live. Web gates green: typecheck, lint, format:check, 2144/2144 tests, build                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
