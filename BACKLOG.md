...
Last reconciled: **2026-08-13** (post v2.93.0 shipment, headless installed-update verification, and the live heal probe)
...
| Branch                  | `main`; release tag `v2.93.0` points at `7897d8e`; nothing follows the tag but this reconciliation                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Source version          | `2.93.0` in all five version-bearing files; `node scripts/bump-version.mjs --verify` passed inside the release run                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Release commit / tag    | `v2.93.0` points at release commit `7897d8e` ("The header takes its actions, and narrow docks breathe")                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Latest public release   | `v2.93.0`, published 2026-08-13; downloaded installer and `SHA256SUMS.txt` agree on SHA-256 `90c43892ef7a5303256ce89ce9980c046ce003248015e386bcc53f736a37054d` (59,895,146 bytes); live `latest.json` serves `2.93.0`, names the v2.93.0 setup asset, and carries a 420-character signature                                                                                                                                                                                                                                             |
...
| CI / release            | `CI` green on every commit through `8cde52a`; `Release installers` run `31650691227` completed `success` and published v2.93.0; `npm audit` reports 0 vulnerabilities                                                                                                                                                                                                                                                                                                                                                                   |
| Installed desktop app   | `2.93.0` (`Beatform.exe` `ProductVersion` and `FileVersion`). Updated 2026-08-13 headlessly with the SHA-verified release installer and the updater's own arguments (`/P /UPDATE`) after the owner declined UI automation; two isolated WebView2 profiles passed on their first run against the updated bundle: `http://tauri.localhost/`, title `Beatform`, one canvas, WebGPU available                                                                                                                                               |
| Uninstall registry      | `2.93.0` exactly (ALIGN-002 check). **The self-heal was also proven LIVE on this binary:** DisplayVersion set to a `0.0.1-heal-probe` sentinel, one boot of the installed app, read back `2.93.0`. **Correction to the 2026-08-12 row:** before this update the entry read `2.82.0`, not the "verified 2.92.1" recorded then — with a proven-working heal, a booted installed 2.92.1 cannot leave 2.82.0 behind, so that verification cannot have run against this hive as recorded. No code defect; a truth-audit miss, corrected here |
