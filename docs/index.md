...
2. **Drop your track** anywhere on the window (MP3/FLAC/WAV/OGG/M4A). The app
   ...
   real beats.
3. **Pick a look.** Nineteen visual modes across the top; each ships six to
...
4. **Make it yours.** Press **G** for **Visuals**, the dock on the right —
   ...
   motion_, _Looks & themes_, _Sync_, _Modulation_, _Scene_, _Text_, _Live_.
   _Mode_ holds every control of the active visual, grouped, with each
   group's internal constants folded behind its own expert line; _Scene ▸
