...
fn preset(uv: vec2f) -> vec4f {
  ...
  //
  // Mirroring the sweep would remove the seam by construction, but it would
  // also make the ring bilaterally symmetric, which is not what this mode is.
  let specX = fract(ang / TAU + 0.5);
  // Distance to the joint, in spectrum-domain units — zero at the wrap from
  // BOTH sides, so one weight serves the two halves of the crossfade.
  let seamK = smoothstep(0.0, 0.09, min(specX, 1.0 - specX));
  ...
  // Same crossfade as the ring's own spectrum above — peaks[] wraps at the
  // same joint, and a crown that jumped from treble-peak to bass-peak would
  // leave a colour seam along the exact angle the geometry no longer has one.
