...
export const starfield: PresetDef = {
  ...
  wgsl: /* wgsl */ `
// Curl of a scalar noise field => a smooth divergence-free drift so neighbours
// move as one current instead of on uncorrelated orbits (docs/VISUAL-DESIGN.md
// section 4). Cheapened to noise2, this only needs to be smooth, not detailed.
fn curlFlow(pp: vec2f) -> vec2f {
  let e = 0.08;
  let dx = (noise2(pp + vec2f(e, 0.0)) - noise2(pp - vec2f(e, 0.0))) / (2.0 * e);
  let dy = (noise2(pp + vec2f(0.0, e)) - noise2(pp - vec2f(0.0, e))) / (2.0 * e);
  return vec2f(dy, -dx);
}
...
fn preset(uv: vec2f) -> vec4f {
  ...
  let nebT = warpFbm(p * 0.5 + u.time * 0.02, 0.7 + u.mid * 0.9);
