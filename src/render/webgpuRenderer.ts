...
/** Mesh-3D uniform: mat4 viewProj (64) + 31 scalar lanes (124) = 188 bytes,
...
struct M3U {
  ...
  treble: f32, waveCount: f32, terrainSmooth: f32,
}
@group(0) @binding(0) var<uniform> m: M3U;
@group(0) @binding(1) var<storage, read> bins: array<f32>;
// The shared fixed-size waveform buffer (the fragment ABI's binding 4), for
// the Waveform terrain layout: heights read TIME-domain samples there
// instead of spectrum bins.
...
/** Waveform sampled at x in 0..1, linear interpolation, -1..1 — the fragment
 * prelude's waveAt(), embedded because the mesh module is its own compilation
 * unit with its own tiny ABI (the hsl2rgb/tonemap arrangement). */
fn m3_waveAt(x: f32) -> f32 {
  let n = m.waveCount;
  let fi = clamp(x, 0.0, 0.999) * (n - 1.0);
  let i = u32(fi);
  let j = min(i + 1u, u32(n) - 1u);
  return mix(wave[i], wave[j], fract(fi));
}

/**
 * The Waveform layout's ground level at a raster position: a five-tap tent
 * filter of the RECTIFIED waveform, then a square root.
 *
 * Rectified, because a mesh column can only extrude UP — the signed wave
 * folds into ridges where the music is loud and a plain where it is silent
 * (silence must read as flat ground, never a half-height plateau). Filtered
 * BEFORE the shaping so opposite-sign neighbours cannot cancel: the tent
 * averages an envelope, not a signal. The radius rides Terrain smoothing —
 * at 0 all five taps collapse onto one sample (weights sum to 1, an exact
 * pass-through) and cymbal-band content stands as raw spikes; wider settings
 * roll it into dunes. And the square root is the terrain's own calibration,
 * the way Spectro Falls squares its brightness: bins are already perceptual,
 * raw PCM is linear amplitude, and without the lift a -14 LUFS master reads
 * as foothills on an empty plain.
 */
fn terrainLevel(t: f32) -> f32 {
  let d = m.terrainSmooth * 0.01;
  let a = abs(m3_waveAt(t)) * 0.4
        + (abs(m3_waveAt(t - d)) + abs(m3_waveAt(t + d))) * 0.2
        + (abs(m3_waveAt(t - d * 2.0)) + abs(m3_waveAt(t + d * 2.0))) * 0.1;
  return sqrt(clamp(a, 0.0, 1.0));
}
...
fn vs_mesh(
  ...
  let dx = col - half;
  let dz = row - half;
  // Spectral position 0..1 of this column — the Layout enum picks the
  ...
  // around the centre; fract wraps it into Archimedean arms. Waveform is the
  // raster mapping AGAIN — but fr addresses TIME through the triggered
  // window rather than frequency, so the wave snakes across the grid row by
  // row and a bass cycle spans many rows: genuine 2D relief, not a wall.
  let rr = length(vec2f(dx, dz)) / max(m.grid * 0.5, 1.0);
  var fr = rr;
  if (m.binMap > 2.5) {
    fr = f32(ii) / max(m.grid * m.grid - 1.0, 1.0);
  } else if (m.binMap > 1.5) {
    fr = fract(rr - atan2(dz, dx) * 0.15915494);
  } else if (m.binMap > 0.5) {
    fr = f32(ii) / max(m.grid * m.grid - 1.0, 1.0);
  }
  let bi = u32(clamp(fr, 0.0, 0.999) * m.binCount);
  // The cell's LEVEL: the spectrum bin on the three spectrum layouts, the
  // rectified waveform on Terrain. One value that everything downstream —
  // height, shading height, hue lift, band mix, hot tops — reads, so the
  // terrain inherits the whole lighting/reaction rig unchanged, and the
  // spectrum layouts keep the identical load and multiply chain (level IS
  // bins[bi] there — bit-exact with the pre-terrain expression).
  var level = bins[bi];
  if (m.binMap > 2.5) { level = terrainLevel(fr); }
  ...
  let h = level * m.heightScale * (0.7 + m.drive * m.driveHeight) + 0.03;
  ...
  let hLit = level * m.heightScale * 0.7 + 0.03;
  var out: VOut;
...
export class WebGPURenderer implements Renderer {
  ...
  private constructor(canvas: HTMLCanvasElement | OffscreenCanvas, device: GPUDevice) {
    ...
    this.mesh3dLayout = device.createBindGroupLayout({
      entries: [
        ...
        // The fixed-size waveform buffer, for the Waveform terrain layout.
  ...
  private renderMesh3d(
    ...
    F[43] = f.mid;
    F[44] = f.treble;
    // The mesh binds the shared fixed-size waveBuf, so its count is the
    // constant — not f.waveform.length, which is the pre-downsample source.
    F[45] = WAVE_POINTS;
    F[46] = g("terrainSmooth");
    ...
    if (!this.mesh3dBind) {
      this.mesh3dBind = this.device.createBindGroup({
        ...
        entries: [
          ...
          { binding: 1, resource: { buffer: this.binsBuf! } },
          // waveBuf never resizes (fixed WAVE_POINTS), so the binsBuf-resize
          // invalidation above stays the only rebind trigger.
